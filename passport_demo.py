...
############用密码对数据进行TDES加密##################
from pyDes import * #该子程序单独在项目文件夹中存放
import binascii
...
#########################
'''
def SecureCommunication(ID):
    #########根据ID选择EF.文件####################
    #########参数ID必须是字符串#######
    global SSC,SKa1,SKb1,SKa2,SKb2
    ##########先确定文件名和标签##############################
    if ID=='011E':
        name='EF.COM'
        tag='60'
    ...
    print CC1.upper()==DO8Er.upper()
    #############用KSenc解密DO87###############
    data1=iKencTDES(DO87[6:],SKa1,SKb1)[0:8]#解密后的数据还得把填充部分去掉       
    ...
    print '16进制为 : '+hex(L)
    print '还需要读的长度为 ： '+str(L-4)
    print '十六进制为 ： '+hex(L-4)

    ##################################
    ##################################
    ##################################
    ############# Read Binary 命令从偏移4开始剩下字节####################
    print '################### Read Binary 命令从偏移4开始剩下的字节####################'
    ###################填充命令报头######################
    ...
    #############用KSenc解密DO87###############
    data2=iKencTDES(DO87[i+2:],SKa1,SKb1)#还得去掉填充部分
    ...
    print 'ID is : '+ID
    print 'name is : '+name
    print '最终结果 '+name +'('+tag+')'+'is : '+finaldata.upper()
############################################################################
############################################################################
############################################################################
############################################################################
IDlist=['011E','011D','0101','0102','0103','0104','0105','0106','0107','0108','0109','010A','010B','010C','010D','010E','010F','0110']
IDlist=['0108','0109','010A','010B','010C','010D','010E','010F','0110']
################### 待删除------------------------
for ID in IDlist:
    SecureCommunication(ID)
    print '''############################################################
                        读取下一个文件
############################################################'''
print '全部读取完毕'
